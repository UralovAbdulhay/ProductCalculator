...
public class ProjectEndSController implements Initializable {
    ...
    @FXML
    private JFXTextField ndsLiXarajatStavka;

    @FXML
    private JFXTextField ndsLiXarajat;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        ndsLiXarajatStavka.setTextFormatter(new TextFormatter<Double>(filter));

    }


    @FXML
    void ddpLiXarajatStavkaAction(KeyEvent event) {
        try {
    ...
    @FXML
    void ndsLiXarajatStavkaAction(KeyEvent event) {
        try {
            project.setPrNdsliXarStavka(Double.parseDouble(ndsLiXarajatStavka.getText()) / 100);
            init2();
        } catch (NumberFormatException e) {
            System.out.println(e.getMessage());
        }
    ...
    private void init() {
        ...
        ndsLiXarajatStavka.setText((project.getPrNdsliXarStavka() * 100) + "");
        init2();
    }
    ...
    private void init2() {
        ...
        ndsLiXarajat.setText(project.getPrNdsliXarajatlarStr());
