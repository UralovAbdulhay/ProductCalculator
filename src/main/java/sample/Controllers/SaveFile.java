...
//            worksheet.getCell(s).getStyle().setHorizontalAlignment(HorizontalAlignmentStyle.CENTER);
        ...
        if (project.getPrFormulaNum() == 0) {
            endClumn = 29;
        } else if (project.getPrFormulaNum() == 1) {
            endClumn = 31;
        ...
        CellRange tabLab = CellRange.zzaInternal(worksheet, 14, 1, 14, endClumn);
        ...
        worksheet.getCell(row, 17).setValue("Цена CIP (SUM) \n" + project.getPrStavkalar().getStUSD_USZ() + " sum");
        worksheet.getCell(row, 18).setValue("Сумма CIP (SUM) ");

        if (project.getPrFormulaNum() < 2) {
            worksheet.getCell(row, 19).setValue("Там. сборы \n" + (project.getPrStavkalar().getStBojxona() * 100) + " %");
            worksheet.getCell(row, 20).setValue("Пошлина");
            worksheet.getCell(row, 21).setValue("Cумма пошлин");
            worksheet.getCell(row, 22).setValue("Акциз");
            worksheet.getCell(row, 23).setValue("Cумма Акциз");
            worksheet.getCell(row, 24).setValue("НДС 1 \n" + (project.getPrStavkalar().getStNDS1S() * 100) + " %");
            worksheet.getCell(row, 25).setValue("Приход цена");
            worksheet.getCell(row, 26).setValue("Приход Сумма");
            worksheet.getCell(row, 27).setValue("Cтавка DDP");
            worksheet.getCell(row, 28).setValue("Цена  DDP ");
            worksheet.getCell(row, 29).setValue("Сумма  DDP ");


            if (project.getPrFormulaNum() == 1) {
                worksheet.getCell(row, 30).setValue("Цена c НДС 2 \n" + (project.getPrStavkalar().getStNDS2() * 100) + " %");
                worksheet.getCell(row, 31).setValue("Сумма с НДС");
            }
        }

        for (
                TovarZakaz zakaz : project.getProjectZakazList()) {
            ...
            worksheet.getCell(row, 18).setValue(zakaz.getZakazCIPSummUSZ());
            worksheet.getCell(row, 18).getStyle().setNumberFormat("#,##0.00 [$UZS]");

            if (project.getPrFormulaNum() < 2) {
                worksheet.getCell(row, 19).setValue(zakaz.getZakazBojYigini());
                worksheet.getCell(row, 19).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 20).setValue(zakaz.getTovarPoshlina());
                worksheet.getCell(row, 20).getStyle().setNumberFormat("0%");
                worksheet.getCell(row, 21).setValue(zakaz.getZakazPoshlinaSumm());
                worksheet.getCell(row, 22).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 22).setValue(zakaz.getTovarAksiz());
                worksheet.getCell(row, 22).getStyle().setNumberFormat("0%");
                worksheet.getCell(row, 23).setValue(zakaz.getZakazAksizSumm());
                worksheet.getCell(row, 23).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 24).setValue(zakaz.getZakazNDS1Narxi());
                worksheet.getCell(row, 24).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 25).setValue(zakaz.getZakazKelishNarxi());
                worksheet.getCell(row, 25).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 26).setValue(zakaz.getZakazKelishSumm());
                worksheet.getCell(row, 26).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 27).setValue(zakaz.getTovarDDP());
                worksheet.getCell(row, 28).setValue(zakaz.getZakazDDPnarxi());
                worksheet.getCell(row, 28).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                worksheet.getCell(row, 29).setValue(zakaz.getZakazDDPsumm());
                worksheet.getCell(row, 29).getStyle().setNumberFormat("#,##0.00 [$UZS]");

                if (project.getPrFormulaNum() == 1) {
                    worksheet.getCell(row, 30).setValue(zakaz.getZakazNDS2liNarxi());
                    worksheet.getCell(row, 30).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                    worksheet.getCell(row, 31).setValue(zakaz.getZakazNDS2liSumm());
                    worksheet.getCell(row, 31).getStyle().setNumberFormat("#,##0.00 [$UZS]");
                }
            }
        }


        for (int i = 1; i <= endClumn; i++) {
        ...
        CellRange cells = CellRange.zzaInternal(worksheet, startRow, startCol, row, startCol + endClumn - 1);
        cells.getStyle().

                getBorders().

                setBorders(
                        MultipleBorders.all(), SpreadsheetColor.

                                fromColor(Color.BLACK), LineStyle.THIN
                );
